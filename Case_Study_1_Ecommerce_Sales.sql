...
select O.order_id ,O.order_date, o.total_amount ,C.full_name from Orders O join Customers C on O.customer_id = C.customer_id;


/* List all orders with product details included*/
select D.order_id, P.product_name , P.category,P.price from Order_details D join Products P on D.order_id = P.product_id;



/*Show the total sales amount for each customer.*/
select O.customer_id,C.full_name,SUM(O.total_amount) as Total_sales  from Orders O join Customers C on O.customer_id = C.customer_id group by C.customer_id,C.full_name;

/* Identify the top 2 customers who spent the most */
select O.customer_id,C.full_name,SUM(O.total_amount) as Total_sales  from Orders O join Customers C on O.customer_id = C.customer_id group by C.customer_id,C.full_name order by Total_sales DESC limit 2;

/* Find the most popular product (the product that appears most in orders)*/
SELECT P.product_id, P.product_name, COUNT(O.product_id) AS total_sales
FROM Products P
...
select * from Order_details where product_id = 4;
...
select P.category, Sum( P.price * O.quantity )as revenu from products P inner join Order_details O On P.product_id = O.product_id group by P.category;
...
select C.full_name , O.customer_id ,count(O.order_id) as total_orders from Customers C inner join Orders O on C.customer_id = O.customer_id group by C.customer_id;
